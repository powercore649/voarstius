...
export default function Analytics({ selectedGuild }) {
  ...
  if (loading) return <div className="loader">Chargement des analytics…</div>;
  ...
  return (
    <div className="analytics-page animate-fade-in">
      ...
      <div className="command-hero glass-panel">
        <div>
          <p className="command-eyebrow">Données en direct · /api/moderation/{selectedGuild}/stats</p>
          <h2 className="glow-text">Analytics</h2>
          <p className="subtitle">Statistiques de modération réelles de ton serveur Discord.</p>
        </div>
      ...
      <div className="glass-panel analytics-toolbar">
        <span className="analytics-toolbar-label">Période :</span>
        {[7, 14, 30].map(r => (
          <button key={r} className={`analytics-range-btn${range === r ? ' active' : ''}`} onClick={() => setRange(r)}>
            {r}j
          </button>
        ))}
        <button className="btn-icon" onClick={fetchStats} title="Actualiser" style={{ marginLeft: 'auto' }}>
      ...
      <div className="analytics-kpi-grid">
        <div className="glass-panel analytics-kpi-card">
          <p className="analytics-kpi-label">Actions sur {range}j</p>
        ...
        <div className="glass-panel analytics-kpi-card">
          <p className="analytics-kpi-label">Pic d'activité</p>
        ...
        <div className="glass-panel analytics-kpi-card">
          <p className="analytics-kpi-label">Jours actifs</p>
          <h3 className="analytics-kpi-value" style={{ color: '#33b5e5' }}>{activeDays}</h3>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: '4px 0 0' }}>sur {dailyData.length} jours</p>
        </div>
        <div className="glass-panel analytics-kpi-card">
          <p className="analytics-kpi-label">Warnings totaux</p>
          <h3 className="analytics-kpi-value" style={{ color: '#ff8800' }}>{stats.totalWarnings ?? '—'}</h3>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: '4px 0 0' }}>cumulés</p>
        </div>
      </div>
      ...
      <div className="glass-panel analytics-main-chart">
        <h3 className="analytics-chart-title">
          <i className="fa-solid fa-chart-area" /> Activité quotidienne ({range} jours)
        </h3>
      ...
      <div className="status-grid-two">
        <div className="glass-panel" style={{ padding: '20px 24px' }}>
          <h3 className="analytics-chart-title"><i className="fa-solid fa-chart-pie" /> Répartition des actions</h3>
          {breakdown.length === 0 ? (
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Aucune action enregistrée.</p>
          ) : (
        ...
        <div className="glass-panel" style={{ padding: '20px 24px' }}>
          <h3 className="analytics-chart-title"><i className="fa-solid fa-shield" /> Top Modérateurs</h3>
          {!stats.topModerators?.length ? (
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Pas encore de données.</p>
          ) : (
      ...
      {stats.topTargets?.length > 0 && (
        <div className="glass-panel" style={{ padding: '20px 24px' }}>
          <h3 className="analytics-chart-title"><i className="fa-solid fa-user-xmark" /> Utilisateurs les plus sanctionnés</h3>
